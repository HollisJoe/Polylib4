...
class BBox {
public:
    // BBox(); // g++リンクエラー　後でinit()で初期化してもエラーとなる
               //   `PolylibNS::BBox::BBox()' に対する定義されていない参照です
    BBox()
    {
        PL_REAL s = 1e10;
        min=(PL_REAL)(s);
    ...
    {
        min.x = _minx; min.y = _miny; min.z = _minz;
        max.x = _maxx; max.y = _maxy; max.z = _maxz;
    }

    BBox(PL_REAL _min[3], PL_REAL _max[3])
    {
        min.x = _min[0]; min.y = _min[1]; min.z = _min[2];
        max.x = _max[0]; max.y = _max[1]; max.z = _max[2];
    }
    ...
    Vec3<PL_REAL> center() const { return (PL_REAL)(.5) * (min + max); }
    

    Vec3<PL_REAL> size() const { return max - min; }

    PL_REAL xsize() const { return max.x - min.x; }
    PL_REAL ysize() const { return max.y - min.y; }
    PL_REAL zsize() const { return max.z - min.z; }
    PL_REAL length(const AxisEnum& axis) const  { return max[axis] - min[axis]; }
    PL_REAL diameter() const { return (max - min).length(); }
    AxisEnum getMaxAxis(PL_REAL& length) const
    {
        Vec3<PL_REAL> len = size();
        AxisEnum axis;
        if (len.x > len.y) {
            if (len.x > len.y)  axis = AXIS_X;
            else                axis = AXIS_Z;
        }
        else {
            if (len.y > len.z)  axis = AXIS_Y;
            else                axis = AXIS_Z;
        }
        length = len[axis];
        return axis;
