...
#if 0
    for(int i=0; i<3; i++){
        m_bbox.add(tri->get_vertexes()[i]);
    }
#else
    m_bbox = tri->get_bbox( true ); // NptTriangleのため変更
#endif
    m_pos = m_bbox.center();
}
...
vector<Triangle*>* VTree::search(
    ...
    vector<Triangle*> *tri_list = new vector<Triangle*>;
    tri_list->reserve( vlist.size() );
    for (; itr != vlist.end(); itr++) {
//#ifdef MEMCOPY_TYPE
...
// public /////////////////////////////////////////////////////////////////////
const Triangle* VTree::search_nearest(
...
// public /////////////////////////////////////////////////////////////////////
const Triangle* VTree::search_nearest_recursive(
            VNode          *vn,
            const Vec3<PL_REAL>&    pos
    ) const 
{
    if (vn->is_leaf()) {
        const Triangle* tri_min = 0;
        ...
        for (; itr != vn->get_vlist().end(); itr++) {
            const Triangle* tri = (*itr)->get_triangle();
            const Vec3<PL_REAL> *v = tri->get_vertexes();
        ...
        const Triangle* tri = search_nearest_recursive(vn1, pos);
        if (tri) {
...
try{
    ...
    if (vn->is_leaf()) {
        ...
        for (; itr != vn->get_vlist().end(); itr++) {
            ...
            if (every == true) {
                bool iscontain = true;
                const Vec3<PL_REAL> *temp = (*itr)->get_triangle()->get_vertexes();
                for (int i = 0; i < 3; i++) {
