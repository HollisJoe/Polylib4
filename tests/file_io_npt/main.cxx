...
int main(int argc, char** argv )
{
    ...
    if( myrank == 0 )  {
        ...
        std::string file_name_bin = "sphere_1_out_bin.nptb";
        fmt_out = PolygonIO::FMT_NPT_B;
        ...
        //---- ポリゴン座標の確認　-------
        PL_REAL eps_l = 0.001;  // 長さ系
        PL_REAL eps_v = 0.0001;  // 法線ベクトル系
        for(int i=0; i<num_tri_sphere_1; i++ )  {
            ...
            for(int j=0; j<3; j++ )  {
                if( fabs(v1[j].x-v2[j].x) > eps_l ) err_flg=true; 
                if( fabs(v1[j].y-v2[j].y) > eps_l ) err_flg=true; 
                if( fabs(v1[j].z-v2[j].z) > eps_l ) err_flg=true; 
                if( fabs(v1[j].x-v3[j].x) > eps_l ) err_flg=true; 
                if( fabs(v1[j].y-v3[j].y) > eps_l ) err_flg=true; 
                if( fabs(v1[j].z-v3[j].z) > eps_l ) err_flg=true; 
            }
        ...
        // 長田パッチパラメータチェック
        for(int i=0; i<num_tri_sphere_1; i++ )  {
            NptTriangle* npt_sphere_1 = dynamic_cast<NptTriangle*>(tri_list_sphere_1  [i]);
            NptTriangle* npt_sphere_2 = dynamic_cast<NptTriangle*>(tri_list_sphere_2  [i]);
            NptTriangle* npt_sphere_3 = dynamic_cast<NptTriangle*>(tri_list_sphere_bin[i]);
            NpatchParam* npt1 = npt_sphere_1->get_npatch_param();
            NpatchParam* npt2 = npt_sphere_2->get_npatch_param();
            NpatchParam* npt3 = npt_sphere_3->get_npatch_param();

            if( fabs(npt1->cp_side1_1.x-npt2->cp_side1_1.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_1.y-npt2->cp_side1_1.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_1.z-npt2->cp_side1_1.z) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_1.x-npt3->cp_side1_1.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_1.y-npt3->cp_side1_1.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_1.z-npt3->cp_side1_1.z) > eps_v ) err_flg=true; 

            if( fabs(npt1->cp_side1_2.x-npt2->cp_side1_2.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_2.y-npt2->cp_side1_2.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_2.z-npt2->cp_side1_2.z) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_2.x-npt3->cp_side1_2.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_2.y-npt3->cp_side1_2.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side1_2.z-npt3->cp_side1_2.z) > eps_v ) err_flg=true; 

            if( fabs(npt1->cp_side2_1.x-npt2->cp_side2_1.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_1.y-npt2->cp_side2_1.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_1.z-npt2->cp_side2_1.z) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_1.x-npt3->cp_side2_1.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_1.y-npt3->cp_side2_1.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_1.z-npt3->cp_side2_1.z) > eps_v ) err_flg=true; 

            if( fabs(npt1->cp_side2_2.x-npt2->cp_side2_2.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_2.y-npt2->cp_side2_2.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_2.z-npt2->cp_side2_2.z) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_2.x-npt3->cp_side2_2.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_2.y-npt3->cp_side2_2.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side2_2.z-npt3->cp_side2_2.z) > eps_v ) err_flg=true; 

            if( fabs(npt1->cp_side3_1.x-npt2->cp_side3_1.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_1.y-npt2->cp_side3_1.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_1.z-npt2->cp_side3_1.z) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_1.x-npt3->cp_side3_1.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_1.y-npt3->cp_side3_1.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_1.z-npt3->cp_side3_1.z) > eps_v ) err_flg=true; 

            if( fabs(npt1->cp_side3_2.x-npt2->cp_side3_2.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_2.y-npt2->cp_side3_2.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_2.z-npt2->cp_side3_2.z) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_2.x-npt3->cp_side3_2.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_2.y-npt3->cp_side3_2.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_side3_2.z-npt3->cp_side3_2.z) > eps_v ) err_flg=true; 

            if( fabs(npt1->cp_center.x-npt2->cp_center.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_center.y-npt2->cp_center.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_center.z-npt2->cp_center.z) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_center.x-npt3->cp_center.x) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_center.y-npt3->cp_center.y) > eps_v ) err_flg=true; 
            if( fabs(npt1->cp_center.z-npt3->cp_center.z) > eps_v ) err_flg=true; 

            if( err_flg )  {
                PL_ERROSH << "#### ERROR nagata patch parameter i="<<i <<endl;
                PL_DBGOSH << "#### ERROR nagata patch parameter i="<<i <<endl;
                //break;
                exit(1);
            }
        }
    ...
    if( myrank == 0 )  {
        std::cerr<<"------------------------------------------" <<endl;
        std::cerr<<"   PASS :  file_io_npt (Normal End)" <<endl;
        std::cerr<<"------------------------------------------" <<endl;
    }
